    ...
    try:
        ...
        if freecheck == 1:
            reply_markup = InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("💎 Get Premium to Forward", url="https://t.me/xTaR_Force_Sub")]
                ]
            )
...
async def rename_file(file, sender, caption=None):
    ...
    base_name = re.sub(r'@\w+', '[꧁༺ȶɦɛ ʊռӄռօառ ֆȶǟʀ༻꧂](https://t.me/xTaR_Force_Sub)', base_name)  # Replace mentions
    for word in delete_words:
...
def progress_callback(done, total, user_id):
    ...
    final = (
        f"╭──────────────────╮\n"
        f"│     **[꧁༺ȶɦɛ ʊռӄռօառ ֆȶǟʀ༻꧂](https://t.me/xTaR_Force_Sub) Uploader__**       \n"
        f"├──────────\n"
        f"│ {progress_bar}\n\n"
...
def dl_progress_callback(done, total, user_id):
    ...
    final = (
        f"╭──────────────────╮\n"
        f"│     ** [꧁༺ȶɦɛ ʊռӄռօառ ֆȶǟʀ༻꧂](https://t.me/xTaR_Force_Sub) Downloader__**       \n"
        f"├──────────\n"
        f"│ {progress_bar}\n\n"
        f"│ **__Progress:__** {percent:.2f}%\n"
        f"│ **__Done:__** {done_mb:.2f} MB / {total_mb:.2f} MB\n"
